...
from models.CTO_Generator import CTO_Generator
from models.CTO_CML import CTO_CombineMLP
from models.CTO_Discriminators import ClassAttacker, CTO_Discriminator
...
def main():
    ...
    init_parser.add_argument('--model_name', type=str, default='MF',
    ...
    if args.log_file == '../log/log.txt':
        if args.no_filter:
            ...
            args.log_file = '../log/cto/%s.txt' % (log_file_name)
    ...
    if args.filter_mode=='cto':
        model.load_model()
        cto_target_disc_dict = {}
        for feat_idx in data_reader.feature_info:
            cto_target_disc_dict[feat_idx] = \
                CTO_Discriminator(args.u_vector_size, data_reader.feature_info[feat_idx],
                              model_dir_path=os.path.dirname(args.model_path), target=True)
            cto_target_disc_dict[feat_idx].apply(cto_target_disc_dict[feat_idx].init_weights)
            if torch.cuda.device_count() > 0:
                cto_target_disc_dict[feat_idx] = cto_target_disc_dict[feat_idx].cuda()

        # create discriminators
        cto_fair_disc_dict = {}
        for feat_idx in data_reader.feature_info:
            cto_fair_disc_dict[feat_idx] = \
                CTO_Discriminator(args.u_vector_size, data_reader.feature_info[feat_idx],
                              model_dir_path=os.path.dirname(args.model_path))
            cto_fair_disc_dict[feat_idx].apply(cto_fair_disc_dict[feat_idx].init_weights)
            if torch.cuda.device_count() > 0:
                cto_fair_disc_dict[feat_idx] = cto_fair_disc_dict[feat_idx].cuda()

        # create feature_generators
        cto_feature_generator_dict = {}
        for feat_idx in data_reader.feature_info_uid:
            cto_feature_generator_dict[feat_idx - 1] = \
                CTO_Generator(args.u_vector_size, data_reader.feature_info_uid[feat_idx],
                          model_dir_path=os.path.dirname(args.model_path))
            cto_feature_generator_dict[feat_idx - 1].apply(cto_feature_generator_dict[feat_idx - 1].init_weights)
            if torch.cuda.device_count() > 0:
                cto_feature_generator_dict[feat_idx - 1] = cto_feature_generator_dict[feat_idx - 1].cuda()

        cto_combine_mlp = CTO_CombineMLP(args.u_vector_size, model_dir_path=os.path.dirname(args.model_path))
        cto_combine_mlp.apply(cto_combine_mlp.init_weights)
        if torch.cuda.device_count() > 0:
            cto_combine_mlp = cto_combine_mlp.cuda()
    ...
    if args.load > 0:
        ...
        if args.filter_mode=='cto':
            model.load_model()
            for idx in cto_feature_generator_dict:
                cto_feature_generator_dict[idx].load_model()
            cto_combine_mlp.load_model()
    if args.train > 0:
        if args.no_filter:
            ...
            runner.train(model, data_processor_dict,
                         ...
                         ex_dp_dict=extra_data_processor_dict, ex_fair_disc_dict=extra_fair_disc_dict,
                         lr_attack=args.lr_attack, l2_attack=args.l2_attack,
                         cto_feature_generator_dict=cto_feature_generator_dict, cto_target_disc_dict=cto_target_disc_dict,
                         cto_fair_disc_dict=cto_fair_disc_dict, cto_combine_mlp=cto_combine_mlp)
    ...
    if args.no_filter:
        ...
        fair_result_dict \
            = runner.train_discriminator(model, extra_data_processor_dict, extra_fair_disc_dict,
                                         args.lr_attack, args.l2_attack,
                                         cto_feature_generator_dict=cto_feature_generator_dict, cto_combine_mlp=cto_combine_mlp)
    ...
    if args.no_filter:
        ...
        if args.filter_mode == 'cto':
            test_result, test_result_dict = runner.eval_multi_combination(model, test_data, args.fix_one,
                                                                          cto_feature_generator_dict=cto_feature_generator_dict,
                                                                          cto_combine_mlp=cto_combine_mlp)
    n_list = 0
