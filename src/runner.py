...
class RecRunner:
    ...
    def parse_runner_args(parser):
        ...
        parser.add_argument('--lambda_weight', type=float,
                            default=1, help='cto: the value of lambda')
        parser.add_argument('--beta_weight', type=float,
                            default=1, help='cto: the value of beta')
    ...
    def train(self, model, dp_dict, skip_eval=0, fix_one=False,
              ...
              cto_feature_generator_dict=None, cto_target_disc_dict=None,
              cto_fair_disc_dict=None, cto_combine_mlp=None):
        ...
        try:
            for epoch in range(self.epoch):
                ...
                if self.no_filter:
                    ...
                    output_dict = self.fit_cto(model, cto_feature_generator_dict, cto_target_disc_dict,
                                           cto_fair_disc_dict, cto_combine_mlp, train_data, epoch=epoch)
                if self.check_epoch > 0 and (epoch == 1 or epoch % self.check_epoch == 0):
                    if self.no_filter:
                        self.check(model, output_dict)
                    else:
                        self.check_cto(model, output_dict)
                    ...
                    if best_result(self.metrics[0], self.valid_results) == self.valid_results[-1]:
                        if self.no_filter:
                            ...
                            for idx in cto_feature_generator_dict:
                                cto_feature_generator_dict[idx].save_model()
                            for idx in cto_target_disc_dict:
                                cto_target_disc_dict[idx].save_model()
                            for idx in cto_fair_disc_dict:
                                cto_fair_disc_dict[idx].save_model()
                            cto_combine_mlp.save_model()
        ...
        if self.no_filter:
            ...
            for idx in cto_target_disc_dict:
                cto_target_disc_dict[idx].load_model()
            for idx in cto_fair_disc_dict:
                cto_fair_disc_dict[idx].load_model()
            cto_combine_mlp.load_model()

    def eval_multi_combination(self, model, data, fix_one=False,
                               cto_feature_generator_dict=None, cto_combine_mlp=None):
        """
    ...
    def evaluate(self, model, batches, mask=None, metrics=None,
        ...
        model.eval()
        if self.filter_mode=='cto':
            for idx in cto_feature_generator_dict:
                cto_feature_generator_dict[idx].eval()
            cto_combine_mlp.eval()
        ...
        for batch in tqdm(batches, leave=False, ncols=100, mininterval=1, desc='Predict'):
            ...
            if self.no_filter:
                ...
                if self.model_name == 'MF' or self.model_name == 'BPR':
                    uids = batch['uid']
                    vectors = model.uid_embeddings(uids)
                elif self.model_name == 'NGCF' or self.model_name == 'LightGCN':
                    seq = batch['seq']
                    vectors = model.log2feats(seq)[:, -1]
                combine_vectors = cto_feature_generator_dict[0](vectors)
                for idx, val in enumerate(mask):
                    feature_vectors = cto_feature_generator_dict[idx + 1](vectors)
                    if val == 1:
                        feature_vectors = feature_vectors * 0
                    combine_vectors = torch.cat((combine_vectors, feature_vectors), dim=1)
                vectors = cto_combine_mlp(combine_vectors)
    ...
    def fit_cto(self, model, feature_generator_dict, target_disc_dict, fair_disc_dict, combine_mlp, batches, epoch=-1):  # fit the results for an input set
        """
        Train the model and the cto model
        :param model: model instance
        :param batches: train data in batches
    ...
    def check_cto(self, model, out_dict):
        """
        Check intermediate results
        :param model: model obj
        :param out_dict: output dictionary
